...
select count(*) from spotify limit 20;
select count(distinct artist) from spotify ;
select distinct album_type from spotify ;
select max(duration_min) from spotify;
delete from spotify where duration_min=(select min(duration_min) from spotify);
select distinct most_played_on from spotify;
select album from spotify where most_played_on='Youtube';
Select avg(duration_min) from spotify;
select most_played_on,count(*) from spotify group by most_played_on;
select * from spotify where likes=(select max(likes) from spotify);
select * from spotify where likes=(select min(likes) from spotify);
